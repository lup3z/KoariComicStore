...
const productCreateValidationMiddleware = [
	...
	body('volumen')
		.notEmpty().withMessage('Tienes que escribir a qué volumen pertenece').bail()
		.isLength({ min: 3 }).withMessage('El volumen debe ser más largo'),
    
	body('autor')
		.notEmpty().withMessage('Tienes que escribir quién es el autor').bail()
		.isLength({ min: 8 }).withMessage('El nombre del autor debe ser más largo'),

	body('artista')
		.notEmpty().withMessage('Tienes que escribir el nombre del artista').bail()
		.isLength({ min: 8 }).withMessage('El nombre del artista debe ser más largo'),

    body('editorial')
		.notEmpty().withMessage('Tienes que escribir el nombre de la editorial').bail()
		.isLength({ min: 5 }).withMessage('El nombre de la editorial debe ser más largo'),

    body('qDePaginas')
		.notEmpty().withMessage('Tienes que escribir la cantidad de páginas').bail()
		.isNumeric(),

    body('colorObyn')
		.notEmpty().withMessage('Tienes que escribir si el producto es a color o blanco & negro').bail()
		.isLength({ min: 4 }).withMessage('La información sobre la tinta del producto debe ser más larga'),

    body('edicion')
		.notEmpty().withMessage('Tienes que escribir el nombre de la edición').bail()
		.isLength({ min: 5 }).withMessage('El nombre de la edición debe ser más largo'),

    body('precio')
		.notEmpty().withMessage('Tienes que escribir el precio').bail()
		.isNumeric(),
